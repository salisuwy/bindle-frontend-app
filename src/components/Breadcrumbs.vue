...
<template>
    <nav aria-label="breadcrumb" :style="props.style" :class="getClass">
        <ol class="breadcrumbs flex flex-row flex-wrap gap-2 sm:gap-4">
            <li v-for="(breadcrumb, index) in breadcrumbs" :key="index" class="breadcrumb-item inline text-sm">
                <span v-if="index>0" class="mr-3">/</span>
                <router-link v-if="breadcrumb.path!==path" :to="breadcrumb.path">{{ breadcrumb.text }}</router-link>
