...
const transitionBeforeClose = () => {
    if (props.overlay) {
        document.getElementsByClassName('transition-modal-overlay')[0].classList.add('transition-modal-overlay-out');
    }
    state.value = "closing";
}
const transitionAfterClose = () => {
    if (props.overlay) {
        ...
        document.getElementsByClassName('transition-modal-overlay')[0].classList.remove('transition-modal-overlay-out');
    }
    state.value = "closed";
    //emit('closed');
}
...
<template>
    <div>
    <div v-if="!props.trigger && !props.noOpenButton" @click="open=true" :class="['inline-block',props.openClass]">
...
<!--    <teleport to="#app"> -->
        ...
        <transition :name="'slide-in-'+direction"
            ...
            <div v-if="open" :class="['transition-modal-'+direction, props.class]"
                 :style="getModalStyles()">
                <div class="transition-modal-close-button" @click="open=false" :style="props.closeButtonStyle"
                     v-if="!props.noCloseButton">
                    <slot name="close">
                        <svg width="26" height="23" viewBox="0 0 26 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M2.59961 1.82422L21.8986 20.1763M2.59961 1.82422L21.8986 20.1763"
                                  :stroke="props.closeButtonColour"
                                  stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M2.54297 20.1143L21.9587 1.88577M2.54297 20.1143L21.9587 1.88577"
                                  :stroke="props.closeButtonColour"
                                  stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </slot>
