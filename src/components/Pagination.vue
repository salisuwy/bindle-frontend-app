...
const getClass = computed(() => {
  return Util.ensureDefault(props.class, [
    'pagination',
    'flex',
    'flex-row',
    'justify-center',
...
const getButtonClass = (pageIdx) =>
  pageIdx === props.modelValue
    ? Util.ensureDefault(props.itemClass, props.itemSelectedClass)
    : props.itemClass;
const getArrowClass = computed(() => Util.ensureDefault(props.itemClass, props.arrowClass));
const getLeftClass = computed(() =>
  props.modelValue > 1
    ? getArrowClass.value
    : Util.ensureDefault(getArrowClass.value, props.arrowDisabledClass)
);
  ...
  <div v-if="rangeItems.length > 1" :class="getClass" :style="props.style">
    <div v-if="props.arrowControls">
      <button
        ...
        :disabled="props.modelValue < 2"
        @click="decrementPage()"
      >
        <slot name="control-decrement">&lt;</slot>
      </button>
    </div>
    <div v-for="(pageIdx, idx) in rangeItems" :key="idx">
      <button
        :class="getButtonClass(pageIdx)"
        :style="props.itemStyle"
        v-if="pageIdx != null"
        @click="clickPage(pageIdx)"
        :data-selected="pageIdx === props.modelValue ? true : null"
        :disabled="pageIdx === props.modelValue"
      >
        {{ pageIdx }}
      </button>
      <div :class="props.itemClass" :style="props.itemStyle" v-else>
        <slot name="control-ellipsis">&hellip;</slot>
      </div>
    </div>
    <div v-if="props.arrowControls">
      <button
        :class="getRightClass"
        :style="props.itemStyle"
        :disabled="props.modelValue >= highestIdx"
        @click="incrementPage()"
      >
        <slot name="control-increment">&gt;</slot>
      </button>
    </div>
  </div>
</template>
