  ...
  <div ref="menuRef" :class="menuClass" :style="cssProps" draggable="false">
    <ul :id="props.id" v-if="props.items" :class="props.listClass" :style="props.listStyle">
      <li v-for="item in menuItems" :key="item.key" :class="getLinkClass(item)">
        ...
        <label :for="item.url">
        <router-link v-if="useRouterLink" @click="clicked(item, $event)"  :class="props.linkClass" :style="props.linkStyle" :to="item.url" :data-active="isActive(item.url)">{{item.item}}</router-link>
        <a v-else @click="clicked(item, $event)" :class="props.linkClass" :style="props.linkStyle" :href="item.url" :data-active="isActive(item.url)">{{item.item}}</a>
        </label>
        <ul v-if="'html' in item" v-html="item.html" :class="props.submenuClass" :style="props.submenuStyle" />
        <ul v-else-if="'children' in item" :class="props.submenuClass" :style="props.submenuStyle">
          <li v-for="subitem in item['children']" :key="subitem.key" class="submenu-item">
            <router-link v-if="useRouterLink" :class="props.linkClass" :style="props.linkStyle" :to="subitem.key" :data-active="isActive(item.url)">{{subitem.item}}</router-link>
            <a v-else :class="props.linkClass" :style="props.linkStyle" :href="subitem.url" :data-active="isActive(item.url)">{{subitem.item}}</a>
          </li>
        </ul>
        <ul v-else-if="hasSlot(Util.snakeCase(item.item))" :class="props.submenuClass" :style="props.submenuStyle"><li>
