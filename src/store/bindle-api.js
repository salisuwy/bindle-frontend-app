...
export const useBindleApiStore = defineStore('bindleApi', () => {
    ...
    const API_ENDPOINT = import.meta.env.VITE_API_ENDPOINT || "https://stage.bindle.co.uk/api/";
    ...
    const getPopularBundles = async (count, is_core=null, level_slug = null, subject_slug = null) => {
    ...
    const getRandomBundles = async (count, is_core=null, level_slug=null, subject_slug=null) => {
        await getBundles()
    ...
    const getFeaturedBooks = async (count, level_id = null, subject_id = null) => {
        await getBooks()
        let booksArray = Object.values(books.value).filter((book) =>
            book.is_featured && (level_id===null || book.level_id===level_id) || (subject_id===null || book.subject_ids.includes(subject_id))
        );
    ...
    const getPopularBooks = async (count, level_id = null, subject_id = null) => {
        ...
        if (level_id !== null) {
            booksArray = booksArray.filter((book) => (book.level_id !== null && book.level_id === level_id));
        }
        if (subject_id !== null) {
            booksArray = booksArray.filter((book) => (book.subject_id !== null && book.subject_ids.includes(subject_id)));
        }
        return Util.getRandom(booksArray, count);
    }
    ...
    return {
        getHomepageData,
        getBooks, getBooksById, getBookById, getBookBySlug, fetchBookBySlug, getBookUrl,
        getBundles, getBundleById, getBundleBySlug, getBestsellingBundles,
        getSubjects, getSubjectById, getSubjectsById, getSubjectBySlug,
        getTypes, getTypeById, getTypesById,
        getLevels, getLevelById, getLevelBySlug,
        getExamboards, getExamboardById,
        getPopularBooks, getFeaturedBooks, getPopularBundles, getPrioritisedSubjectsIndex,
        getRandomBundles, getRandomBooks,
        addSubscriber,
        getSubjectIconClass,
        subjects, types, levels, examboards, books, bundles,
        featuredBookIds,
        bookIdBySlugIndex,
        routingFailed,
    }
