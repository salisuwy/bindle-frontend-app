...
const getTags = computed(()=> {
    if (bundle.value===null || bindleApiStore.examboards===null || bindleApiStore.levels===null || bindleApiStore.subjects===null) {
        return [];
    }
    else {
        const tags=[];
        ...
        <div class=" bundle bg-theme-white py-10 relative">
            <div class="mx-auto max-w-8xl w-full px-6 text-left mb-16">
                ...
                <div class="flex flex-col md:flex-row gap-4 w-full">
                    <div class="w-full md:w-1/2 p-8 relative">
                        <div class="flex flex-row h-fit sticky top-12 bg-theme-pale w-full py-6">
                            <img :src="bundleImages[0]" alt="book image" class="w-1/3 mx-auto scale-70 rotate-5"/>
                            <img :src="bundleImages[1]" alt="book image" class="w-1/3 mx-auto"/>
                            <img :src="bundleImages[2]" alt="book image" class="w-1/3 mx-auto scale-70 -rotate-5"/>
                        </div>
                    </div>
                    <div v-if="bundle" class="w-full md:w-1/2 p-8">
                        <h3 class="text-theme-teal uppercase">{{bundle.is_core_bundle ? 'Core ':''}}Bundle</h3>
                        <h1>{{bundle.title}}</h1>
                        <div class="text-theme-darkgray mb-4">Bindle Experts</div>
                        <div v-if="bundle.price_amount<bundle.price_total_of_books" class="text-theme-darkgray line-through">&pound;{{Util.toFixedDisplay(bundle.price_total_of_books,2)}}</div>
                        <div class="flex flex-row gap-2 my-4 items-end">
                            <div class="text-3xl font-semibold">&pound;{{Util.toFixedDisplay(bundle.price_amount,2)}}</div>
                            <div v-if="('active_discount' in bundle) && bundle.active_discount"><div class="bg-red-600 text-white px-2 py-1 relative rounded text-xs" style="top:-6px">{{bundle.active_discount.percentage}}&percnt;</div></div>
                        </div>
                        ...
                        <div class="mb-8">
                            <button class="bg-theme-teal w-full rounded" @click="addToBasket()">Add to basket - &pound;{{Util.toFixedDisplay(bundle.price_amount,2)}}</button>
                        </div>
                        ...
                        <div class="my-6">
                            <accordion content-class="text-sm text-theme-darkgray2 mt-2"  title-class="linklike" indicator-class="float-right" open>
                                ...
                                <div>
                                    {{bundle.description}}
                                    Currently the bundle descriptions from the database/api are all null.
                                </div>
                            </accordion>
                        </div>
                        ...
                        <div class="my-6">
                            <accordion content-class="text-sm text-theme-darkgray2 mt-2" title-class="linklike" indicator-class="float-right">
                                ...
                                <template #indicator><chevron-icon down class="inline-block" /></template>
                                <table class="info">
                                    <tr><td>Level{{getLevels.length>1 ? 's' : ''}}</td><td>{{getLevels.join(", ")}}</td></tr>
                                    <tr><td>Exam Board{{getExamboards.length>1 ? 's' : ''}}</td><td>{{getExamboards.join(", ")}}</td></tr>
                                    <tr><td>Subject{{getSubjects.length>1 ? 's' : ''}}</td><td>{{getSubjects.join(", ")}}</td></tr>
                                    <tr><td>Type{{getTypes.length>1 ? 's' : ''}}</td><td>{{getTypes.join(", ")}}</td></tr>
                                </table>
                            </accordion>
                        </div>
                ...
                <div v-if="bundle" class="w-full text-center my-8">
                    ...
                    <div class="max-w-2/3 md:w-1/2 mx-auto my-4">Tailored for {{getExamboards.join(" / ")}} Exam Board{{getExamboards.length>0 ? 's' : ''}} deep dive into {{getLevels.join(" / ")}} {{getSubjects.join(" / ")}} to ensure you're well prepared for exam success.</div>
                    <div class="md:mx-8 flex flex-col md:flex-row items-center justify-center mx-auto  gap-4 md:gap-0">
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <img :src="bundleImages[0]" alt="book image" class="w-full h-auto" />
                            <div>
                            </div>
                        </div>
                        ...
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <img :src="bundleImages[1]" alt="book image" class="w-full h-auto" />
                        </div>
                        <div class="hidden md:block md:w-1/12">
                            <plus-icon class="mx-auto" />
                        </div>
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <img :src="bundleImages[2]" alt="book image" class="w-full h-auto"/>
                        </div>
                    </div>
                    <div v-if="books" class="md:mx-8 flex flex-col md:flex-row items-center md:items-start justify-center mx-auto  gap-4 md:gap-0 py-4">
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <div v-if="0 in books">
                                <div class="text-theme-navyblue">{{getBookTypes(0).join("/")}}</div>
                                <div class="mb-4 text-2xl">{{books[0].title}}</div>
                                <router-link v-if="books[0].url" :to="books[0].url"  class="text-theme-teal">View Product</router-link>
                            </div>
                        </div>
                        <div class="hidden md:block md:w-1/12">
                        </div>
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <div v-if="1 in books">
                                <div class="text-theme-navyblue">{{getBookTypes(1).join("/")}}</div>
                                <div class="mb-4 text-2xl">{{books[1].title}}</div>
                                <router-link v-if="books[1].url" :to="books[1].url"  class="text-theme-teal">View Product</router-link>
                            </div>
                        </div>
                        <div class="hidden md:block md:w-1/12">
                        </div>
                        <div class="w-1/2 md:w-3/12 flex flex-col">
                            <div v-if="2 in books">
                                <div class="text-theme-navyblue">{{getBookTypes(2).join("/")}}</div>
                                <div class="mb-4 text-2xl">{{bundle.books[2].title}}</div>
                                <router-link v-if="books[2].url" :to="books[2].url"  class="text-theme-teal">View Product</router-link>
                            </div>
                        </div>
                    </div>
                </div>
