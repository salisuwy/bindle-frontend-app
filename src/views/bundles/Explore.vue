...
const filterSubject = computed({
    ...
    set: (newValue) => {
        ...
        router.push({ query: { ...route.query, subject: newValue }}).catch(err => { console.error(err) });
    }
});
            ...
            <div class="mx-auto max-w-8xl w-full text-left px-6 grid grid-cols-1 md:grid-cols-4 ">
                <div class="titlebar md:col-start-2 md:col-span-3 row-start-2 text-wrap">
                    <div class="flex flex-row pb-4">
                        <h1 class="text-4xl md:text-4xl">Bundles</h1>
                        <div class="md:hidden grow pt-2 cursor-pointer" @click="toggleMobileFilters()"><filters-icon class="ml-auto" /></div>
                    </div>
                </div>

                <div ref="filtersRef" :class="'filters hidden md:flex md:flex-col md:row-start-2 col-start-1 md:row-span-3 text-wrap pr-4 gap-2'+(filtersLoaded ? '' : ' working-spinner pointer-events-none')">
...
<!--                        <div @click="quickFilter('sale')" class="cursor-pointer linklike">Sale</div>-->
                    ...
                    <accordion ref="filterSubjectAccordionRef" content-class="filter-subjects flex flex-col" title-class="md:text-sm lg:text-base cursor-pointer linklike" indicator-class="float-right">
                        ...
                        <div class="radio-links">
                            <div>
                                <input v-model="filterSubject" id="filter-subject-all" class="bindle-radio" type="radio" value="all">
                                <label class="cursor-pointer mr-auto" for="filter-subject-all">All</label>
                            </div>
                            <div v-for="subject in subjects" :key="subject['slug']">
                                <input :id="'filter-subject-'+subject['slug']" v-model="filterSubject" class="bindle-radio" type="radio" name="subject" :value="subject['slug']" />
                    ...
                    <accordion ref="filterLevelAccordionRef" content-class="filter-level flex flex-col" title-class="md:text-xs lg:text-base cursor-pointer linklike" indicator-class="float-right">
                        <template #title><h3 class="inline-block">QUALIFICATION LEVEL</h3></template>
                        <template #indicator><chevron-icon down class="inline-block" /></template>
                        <hr class="hidden md:block w-full my-4"/>
                        <label class="cursor-pointer mr-auto bindle-checkbox"><input v-model="filterLevel" type="checkbox" class="bindle-radio" value="gcse" /> GCSEs</label>
                        <label class="cursor-pointer mr-auto bindle-checkbox"><input v-model="filterLevel" type="checkbox" class="bindle-radio" value="a-level" /> A-Levels</label>
                    </accordion>
                    ...
                    <accordion ref="filterExamboardAccordionRef" content-class="filter-level flex flex-col" title-class="md:text-sm lg:text-base cursor-pointer linklike" indicator-class="float-right">
                        <template #title><h3 class="inline-block">EXAM BOARD</h3></template>
                        <template #indicator><chevron-icon down class="inline-block" /></template>
                        <hr class="hidden md:block w-full my-4"/>
                        <label  v-if="filtersLoaded" v-for="(examboard) in examboards" :key="examboard['slug']" class="bindle-checkbox cursor-pointer mr-auto">
