...
const coreBundle = ref(null);
const workbookBundle = ref(null);
const selected = ref('core');
const topNavRef = ref(null);

onBeforeMount(() => {
    bindleApiStore.getRandomBundles(1, true).then((bundles) => {
        coreBundle.value = bundles[0];
    });
    bindleApiStore.getRandomBundles(1).then((bundles) => {
        workbookBundle.value = bundles[0];
    })
})
...
const getBookImgUrl = (type, idx) => {
    ...
    const books = (type === 'core') ? coreBundle.value['books'] : workbookBundle.value['books'];
    const keys = Object.keys(books);
    const id = keys[idx];
    if (!(id in books) || !('image_url' in books[id])) {
        return placeholderImages[idx];
    }
    return Util.ensureSsl(books[id]['image_url']);
}
</script>
<template>
    <div class="bundle-types flex flex-col mt-12">
        <div ref="topNavRef" class="nav-container flex flex-row gap-0 py-4 px-4 w-full">
            ...
            <button :class="'py-4 w-1/2 rounded-none'+(selected==='workbook' ? ' selected' :'')"
                    @click="selectPane('workbook')">Workbook Bundle
            </button>
        </div>
        <div class="tabbed-pane-container grow pt-12">
            <div v-show="selected==='core'" ref="coreBundlesPaneRef" class="tabbed-pane selected">
                ...
                <p class="leading-6 mt-4 px-2">Dive deep into a single subject with our tailored Core Bundles, blending
                    three essential study resources to ensure you're well-prepared for exam success.</p>
                <div class="flex flex-col lg:flex-row w-full h-fit items-center justify-center gap-6 pt-10 pb-4 px-10">
                    <div class="flex flex-col pt-4">
                        <div class="mx-auto w-1/2 -rotate-5 mb-8">
                            <img :src="getBookImgUrl('core',0)" alt="book image" class="w-full h-auto shadow-1"/>
                        </div>
                        <div
                    ...
                    <div>
                        <div class="mx-auto w-1/2 mb-8">
                            <img :src="getBookImgUrl('core',1)" alt="book image" class="w-full h-auto shadow-1"/>
                        </div>
                        <div
                            class="bg-theme-teal rounded-full w-10 h-10 leading-10 text-theme-white text-2xl mx-auto my-4">
                            1
                        ...
                        <p class="my-4 px-6">A condensed revision guide focusing on the most fundamental information</p>
                    </div>
                    <div>
                        <plus-icon/>
                    </div>
                    <div>
                        <div class="mx-auto w-1/2 mb-8 rotate-5">
                            <img :src="getBookImgUrl('core',2)" alt="book image" class="w-full h-auto shadow-1"/>
                        </div>
                        <div
                            class="bg-theme-teal rounded-full w-10 h-10 leading-10 text-theme-white text-2xl mx-auto my-4">
                            1
                        ...
                        <p class="my-4 px-6">Subject specific end-of-chapter Q&amp;A pages to consolidate your
                            learning</p>
                    </div>
                </div>
            </div>
            <div v-show="selected==='workbook'" ref="workbookBundlesPaneRef" class="tabbed-pane">
                ...
                <p class="leading-6 mt-4 mb-10 px-2">Enhance your exam technique with three subject-specific workbooks,
                    packed with essential exam-style questions.</p>
                <div class="flex flex-col md:flex-row justify-center md:px-10">
                    <div class="mx-auto w-1/2 md:w-auto mb-8 -rotate-5 md:grow">
                        <img :src="getBookImgUrl('workbook',0)" alt="book image"
                             class="mx-auto w-1/2 md:w-fit h-auto shadow-1 md:ml-auto"/>
                    </div>
                    <div class="mx-auto w-1/2 md:w-fit mb-8">
                        <img :src="getBookImgUrl('workbook',1)" alt="book image"
                             class="mx-auto w-1/2 md:w-fit h-auto shadow-1 md:mx-auto"/>
                    </div>
                    <div class="mx-auto w-1/2 md:w-auto mb-8 rotate-5 md:grow">
                        <img :src="getBookImgUrl('workbook',2)" alt="book image"
                             class="mx-auto w-1/2 md:w-fit h-auto shadow-1 md:mr-auto"/>
                    </div>
                </div>
                <div class="bg-theme-teal rounded-full w-10 h-10 leading-10 text-theme-white text-2xl mx-auto my-4">3
                </div>
                <h3 class="text-xl font-medium">Exam Workbooks</h3>
                <p class="my-4 px-6">Subject specific end-of-chapter Q&amp;A pages to consolidate your learning</p>
            </div>
        </div>
        <div class="nav-container flex flex-row gap-0 py-4 px-4 w-full lg:hidden mb-8">
            <button :class="'py-4 w-1/2 rounded-none'+(selected==='core' ? ' selected' :'')"
                    @click="selectPane('core', true)">Core Bundle
            </button>
            <button :class="'py-4 w-1/2 rounded-none'+(selected==='workbook' ? ' selected' :'')"
                    @click="selectPane('workbook', true)">Workbook Bundle
            </button>
        </div>
    </div>
</template>
