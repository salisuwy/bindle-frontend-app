...
<template>
  <div
    class="mx-auto max-w-8xl grid grid-cols-1 md:grid-cols-6 gap-4 text-center"
  >
    <router-link
      v-for="(id, idx) in prioritisedIndex"
      :to="'/' + props.stub + '/' + subjects[id]['slug']"
      :class="subjectClass(idx) + ' flex align-center justify-center'"
    >
      <img
        v-if="idx < 9 && subjects[id]['image_url']"
      ...
      <div
        v-else
        class="w-full flex flex-row p-6 bg-theme-lightgray rounded-xl"
      >
