    ...
    <main class="w-full">
            ...
            <nav aria-label="breadcrumb" class="text-left w-full col-start-1 md:col-span-4 grow">
                <ol class="breadcrumbs flex flex-row flex-wrap gap-2 sm:gap-4">
                    <li v-for="(breadcrumb, index) in getBreadcrumbs" :key="index"
                        ...
                        <router-link v-if="breadcrumb.path !== ''" :to="breadcrumb.path">{{ breadcrumb.text
                            }}</router-link>
        ...
        <div
            ...
            <div class="profile-sidebar w-full md:w-auto pr-0 md:pr-12 mb-6 md:mb-0 flex flex-wrap flex-col">
                <div class="flex"><button class="md:hidden mb-4" @click="toggleMobileMenu">
                        ☰
                    </button></div>

                <div :class="{ 'hidden': !isMobileMenuVisible, 'block': isMobileMenuVisible, 'md:block': true }">
...
const getBreadcrumbs = computed(() => {
    ...
    const addressPageBreadcrumbs = [
        ...
        { text: "My Account", path: `/user/${authStore.user?.id}` },
        { text: "Addresses", path: `/user/${authStore.user?.id}/addresses` },
        { text: `${authStore.state.currentAddress ? 'Address ID: #' + authStore.state.currentAddress?.id : 'New Address'}`, path: `/user/${authStore.user?.id}/addresses/${authStore.state.currentAddress ? authStore.state.currentAddress?.id : 'new'}` }
    ];
