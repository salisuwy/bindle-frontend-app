...
const productUrl = computed(() => {
  ...
  if (props.book.subject_ids.length > 1) {
    return '/' + level.value.slug + '/subjects/' + props.book.slug;
  } else {
...
const getPrice = computed(() => {
  ...
  return props.ebookSelected
    ? Util.toFixedDisplay(props.book.price_ebook, 2)
    : Util.toFixedDisplay(props.book.price_amount, 2);
  ...
  <div class="flex flex-row my-6 items-stretch">
    ...
    <div class="flex flex-col flex-grow px-4 py-4">
      ...
      <div class="flex-grow flex items-center" v-if="productUrl">
        <router-link :to="productUrl" class="underline">{{ props.book.title }}</router-link>
      </div>
    </div>
    <div class="flex flex-col flex-grow px-4 py-4">
      <div class="flex-grow flex items-center">£{{ getPrice }}</div>
