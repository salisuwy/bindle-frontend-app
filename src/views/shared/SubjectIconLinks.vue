...
<template>
  <div class="w-full bg-theme-light pt-6 pb-2">
    <carousel class="subject-icons" button-class="text-theme-gray" :items-to-show="4" :breakpoints="breakpoints"  :transition="1000" :wrap-around="true" :autoplay="2000">
      <slide v-if="subjectLength>0" v-for="(subject, index) in subjects" :key="index" class="subject-link text-theme-white pb-2 select-none" >
        <router-link class='subject-link' :to="props.slug+'?subject='+subject['slug']">
          <div class="rounded-full w-8 h-8 flex items-center justify-center mx-auto">
            <i :class="'fa fa-solid text-theme-teal '+bindleApiStore.getSubjectIconClass(subject['slug'])" />
          </div>
