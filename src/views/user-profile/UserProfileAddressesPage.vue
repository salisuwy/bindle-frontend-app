<template>
  <UserProfileTabLayout>
    <div>
      <div
        v-if="authStore.state.allAddressesLoading"
      ...
      <div v-else>
        <section
          v-if="authStore.state.allAddresses?.length === 0"
          class="flex flex-col px-5 max-w-[273px] mx-auto my-8"
        >
          <img loading="lazy" src="/assets/delivery-address.svg" />
        ...
        >
          ...
          <div
            v-for="(address, addressIndex) in authStore.state.allAddresses"
            :key="address.id"
          ...
          >
            <div class="flex flex-wrap md:items-start justify-between w-full">
              <label :for="`default-address-${address.id}`" class="text-left">
                <ul>
                  <li class="font-medium customer-name">
                    {{ address.first_name?.toUpperCase() }} {{ address.last_name?.toUpperCase() }}
                  </li>
                  <li>
                    {{ address.address1 }} {{ address.address2 }}, {{ address.town }}
                    {{ address.city }}, {{ address.zip }}, {{ address.country }}
                  </li>
                  <li class="text-gray-500">
                    Phone: <span class="text-theme-black font-medium">{{ address.phone }}</span>
                  </li>
                  <li class="text-gray-500">
                    Email: <span class="text-theme-black font-medium">{{ address.email }}</span>
                  </li>
                  <li v-if="address.is_delivery_default === 1" class="mt-1">
                    <span
                      class="text-sm border bg-teal-500 px-2 py-1 text-white uppercase rounded-lg font-medium"
                      >Default</span
                    >
                  </li>
                </ul>
              ...
              <div class="flex mt-6 md:mt-0">
                ...
                <button
                  v-if="address.is_delivery_default !== 1"
