  ...
  <UserProfileLayout>
    <div v-if="!authStore.currentOrderLoading && authStore.currentOrder" class="w-full text-left">
      <div class="flex flex-row justify-between items-start mb-10">
        <div class="flex flex-col">
          ...
          <span class="text-gray-500"
            >Order ID:
            <span class="text-theme-black font-medium"
              >#{{ authStore.currentOrder.uuid }}</span
            ></span
          >
        </div>

        <a
          :href="authStore.currentOrderInvoice"
          target="_blank"
          class="bg-teal-500 text-white px-3 py-2 rounded hover:bg-teal-600 cursor-pointer"
          >Download Invoice</a
        >
      </div>

      <h1
        class="grow text-xl leading-7 text-gray-700 text-left font-medium mb-2"
        :class="{
          'text-teal-500': authStore.currentOrder.status === 'DISP_BY_GARDNER',
        }"
      >
        {{ orderStatusMapping[authStore.currentOrder.status] }}
      </h1>
          ...
          <div>
            ...
            <ul class="break-all md:break-normal">
              <li class="font-medium customer-name">
                ...
                {{ authStore.currentOrder.delivery_last_name?.toUpperCase() }}
              </li>
              <li>
                {{ authStore.currentOrder.delivery_address1 }}
                ...
                {{ authStore.currentOrder.delivery_country }}
              </li>
              <li class="text-gray-500">
                Phone:
                <span class="text-theme-black font-medium">{{
                  authStore.currentOrder.delivery_phone
                }}</span>
              </li>
              <li class="text-gray-500">
                Email:
                <span class="text-theme-black font-medium">{{
                  authStore.currentOrder.delivery_email
                }}</span>
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul class="break-all md:break-normal">
              <li class="font-medium customer-name">
                ...
                {{ authStore.currentOrder.billing_last_name?.toUpperCase() }}
              </li>
              <li>
                {{ authStore.currentOrder.billing_address1 }}
                ...
                {{ authStore.currentOrder.billing_country }}
              </li>
              <li class="text-gray-500">
                Phone:
                <span class="text-theme-black font-medium">{{
                  authStore.currentOrder.billing_phone
                }}</span>
              </li>
              <li class="text-gray-500">
                Email:
                <span class="text-theme-black font-medium">{{
                  authStore.currentOrder.billing_email
                }}</span>
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul>
              <li class="font-bold customer-name">
                {{ authStore.currentOrder.payment_nameoncard?.toUpperCase() }}
              </li>
              <li class="text-gray-500">
                Card Number:
                <span class="text-theme-black font-medium"
                  >**** **** **** {{ authStore.currentOrder.payment_last4 }}</span
                >
              </li>
              <li class="text-gray-500">
                Expiration Date:
      ...
      <div
        class="flex flex-col w-full bg-white rounded-md border border-solid border-zinc-200 max-md:px-5 max-md:mt-9 max-md:max-w-full mb-6 text-left p-8"
      >
        <h2 class="mb-4 border-b pb-3 font-normal">
          Items
          <span v-if="authStore.currentOrder.items.length > 0" class="text-sm text-teal-400"
            >({{ authStore.currentOrder.items.length }})</span
          >
        </h2>
