<template>
  <MinimalLayout>
    <div class="login-container flex justify-center items-center">
      <div
        v-if="!authStore.state.forgotPassword"
        class="login-box w-full max-w-md bg-white rounded-lg shadow-md text-left"
        @keydown.enter="onForgotPassword"
      >
        <h1 class="text-xl font-semibold text-gray-900 border-b px-6 py-4">
          Forgot Your Password?
        </h1>
        <div class="p-6">
          <!-- Email Input -->
          <p class="text-sm mb-3 text-theme-darkgray2">
          ...
          <!-- Continue Button -->
          <button
            class="w-full bg-teal-500 hover:bg-teal-600 text-white font-medium py-2 rounded-md mb-4"
          ...
          <p class="text-sm text-gray-500 text-center mt-1">
            <a
              href="/login"
              class="text-theme-darkgray hover:text-teal-500 focus:text-teal-500 hover:underline"
            >
              <i class="fa fa-angle-left mr-1"></i> Back to Login</a
            >
          </p>
        </div>
      </div>
      <div v-else class="p-6 w-full max-w-md bg-white rounded-lg shadow-md text-center">
        ...
        <button
          class="w-full bg-teal-500 hover:bg-teal-600 text-white font-medium py-2 rounded-md"
