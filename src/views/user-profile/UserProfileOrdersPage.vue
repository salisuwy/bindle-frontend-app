<template>
  <UserProfileLayout>
    <div
      v-if="authStore.allOrdersLoading"
    ...
    <div v-else class="w-full">
      <section
        v-if="authStore.state.allOrders?.length === 0"
      ...
      <div v-else>
        <h2 class="text-left mb-6">
          Your Orders
          <span v-if="ordersCount > 0" class="text-sm text-gray-400">{{ ordersCount }}</span>
        </h2>
        ...
        <div>
          <div
            v-for="order in authStore.state.allOrders"
            :key="order.id"
          ...
          >
            ...
            >
              <h1
                class="grow text-xl leading-7 text-gray-700 text-left font-medium"
                :class="{
              ...
              <span class="text-gray-500 text-right"
                >Order ID: <span class="text-theme-black font-medium">#{{ order.uuid }}</span></span
              >
            </header>
            <hr class="mt-4 h-px border border-zinc-200" />
            <section
              class="px-6 py-8 flex flex-row justify-between items-end max-md:max-w-full flex-wrap"
            >
              <div>
                <p class="pb-6 text-left">Ordered on {{ formattedDate(order.created_at) }}</p>
                <div class="flex flex-row pt-6 pr-6 items-end">
                  <img
                    v-for="image in order.images.slice(0, 2)"
                    :key="image"
                    class="pr-3 w-28"
                    :src="image"
                  />
                  <span v-if="order.images.length > 2" class="pl-4 text-theme-darkgray"
                    >+ {{ order.images.length - 2 }} More</span
                  >
                </div>
              </div>
              <a
                class="bg-transparent text-teal-500 py-0 items-center cursor-pointer mt-6 md:mt-0"
