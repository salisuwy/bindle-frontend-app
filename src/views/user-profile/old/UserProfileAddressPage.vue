<template>
  <UserProfileTabLayout>
    <div>
      <div
        v-if="authStore.state.currentAddressLoading"
      ...
      <div
        v-else
        class="flex flex-col w-full bg-white rounded-md border border-solid border-zinc-200 p-4 md:p-8"
      >
        <div class="flex flex-col items-start">
          ...
          <button
            v-if="authStore.state.currentAddress && route.params.addressId !== 'new'"
            @click="onEditAddress"
            class="hover:bg-teal-600"
          >
            <span v-if="authStore.state.updateAddressLoading"
              >Editing <i class="fa fa-solid fa-circle-notch fa-spin ml-2"></i
            ></span>
            <span v-else>Edit Address</span>
          </button>
          <button v-else @click="onCreateAddress" class="hover:bg-teal-600">
            <span v-if="authStore.state.createAddressLoading"
              >Saving <i class="fa fa-solid fa-circle-notch fa-spin ml-2"></i
            ></span>
            <span v-else>Save Address</span>
          </button>

          <!-- Update Error Message -->
          <p v-if="authStore.state.updateAddressError" class="text-red-500 text-sm mt-2">
            Error: {{ authStore.state.updateAddressError }}
          </p>

          <!-- Create Error Message -->
          <p v-if="authStore.state.createAddressError" class="text-red-500 text-sm mt-2">
            Error: {{ authStore.state.createAddressError }}
          </p>
        </div>
      </div>
    </div>
